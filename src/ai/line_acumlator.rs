...
impl LineInfo{
    ...
    pub fn ad_board_to_halfLineInfo(a:u64, d:u64) -> HalfLineInfo{
        let line_bundle = line::SimplLineEvaluator::analyze_board(a, d);
        let l = line_bundle.2;
        ...
        let l = line_bundle.1;
        let mut two: [u64; 7] = [
            l.0|thr[0], l.1|thr[1], l.2|thr[2], l.3 | l.4|thr[3], l.5 | l.6|thr[4], l.7 | l.8|thr[5], l.9 | l.10 | l.11 | l.12|thr[6]
        ];
        let l = line_bundle.0;
        let mut one: [u64; 7] = [
            l.0|two[0], l.1|two[1], l.2|two[2], l.3 | l.4|two[3], l.5 | l.6|two[4], l.7 | l.8|two[5], l.9 | l.10 | l.11 | l.12|two[6]
        ];
...
pub fn call_negscout_hash_lineinfo(b:&Board, depth: u8, e: &Box<dyn LineInfoEvaluator>, tt_size: u64) -> (Action, f32, SearchStats){
    ...
    for d in (3..=depth).step_by(2){
        ...
        (action, val) = negscout_hash_lineinfo(
            (att, def), &lineinfo, 
            ZOBRIST_INIT_HASH, 0, d, d, -2.0, 2.0, None, best, &mut hashmap, &mut tt, &mut search_stats, &mut rng, e);
...
pub fn negscout_hash_lineinfo(
    ...
    best_move: Option<u8>,
    hashmap: &mut HashMap<u128, TTEntry>,
    tt: &mut TT,
            ...
            if !is_set{
                ...
                if reduction_flag{
                    if !next_fail.is_fail_high(){            
                        ...
                        (next_action, next_fail) = negscout_hash_lineinfo(
                            next_board, next_line_info, hash, 1 - is_att_flag, depth - 1, max_depth, -beta, -alpha, Some(action.trailing_zeros() as usize), next_best_move, hashmap, tt, stats, rng, e
                        );
                        if old_depth < depth - 1{
                            tt.insert_force(hash, TTEntry { hash_hi: (hash >> 32) as u32, fail: next_fail, depth: depth - 1, best_move: new_best_move });
                        }
